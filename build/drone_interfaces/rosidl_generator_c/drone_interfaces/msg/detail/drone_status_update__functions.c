// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from drone_interfaces:msg/DroneStatusUpdate.idl
// generated code does not contain a copyright notice
#include "drone_interfaces/msg/detail/drone_status_update__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


// Include directives for member types
// Member `drone_id`
// Member `status`
#include "rosidl_runtime_c/string_functions.h"

bool
drone_interfaces__msg__DroneStatusUpdate__init(drone_interfaces__msg__DroneStatusUpdate * msg)
{
  if (!msg) {
    return false;
  }
  // drone_id
  if (!rosidl_runtime_c__String__init(&msg->drone_id)) {
    drone_interfaces__msg__DroneStatusUpdate__fini(msg);
    return false;
  }
  // status
  if (!rosidl_runtime_c__String__init(&msg->status)) {
    drone_interfaces__msg__DroneStatusUpdate__fini(msg);
    return false;
  }
  return true;
}

void
drone_interfaces__msg__DroneStatusUpdate__fini(drone_interfaces__msg__DroneStatusUpdate * msg)
{
  if (!msg) {
    return;
  }
  // drone_id
  rosidl_runtime_c__String__fini(&msg->drone_id);
  // status
  rosidl_runtime_c__String__fini(&msg->status);
}

bool
drone_interfaces__msg__DroneStatusUpdate__are_equal(const drone_interfaces__msg__DroneStatusUpdate * lhs, const drone_interfaces__msg__DroneStatusUpdate * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // drone_id
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->drone_id), &(rhs->drone_id)))
  {
    return false;
  }
  // status
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->status), &(rhs->status)))
  {
    return false;
  }
  return true;
}

bool
drone_interfaces__msg__DroneStatusUpdate__copy(
  const drone_interfaces__msg__DroneStatusUpdate * input,
  drone_interfaces__msg__DroneStatusUpdate * output)
{
  if (!input || !output) {
    return false;
  }
  // drone_id
  if (!rosidl_runtime_c__String__copy(
      &(input->drone_id), &(output->drone_id)))
  {
    return false;
  }
  // status
  if (!rosidl_runtime_c__String__copy(
      &(input->status), &(output->status)))
  {
    return false;
  }
  return true;
}

drone_interfaces__msg__DroneStatusUpdate *
drone_interfaces__msg__DroneStatusUpdate__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  drone_interfaces__msg__DroneStatusUpdate * msg = (drone_interfaces__msg__DroneStatusUpdate *)allocator.allocate(sizeof(drone_interfaces__msg__DroneStatusUpdate), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(drone_interfaces__msg__DroneStatusUpdate));
  bool success = drone_interfaces__msg__DroneStatusUpdate__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
drone_interfaces__msg__DroneStatusUpdate__destroy(drone_interfaces__msg__DroneStatusUpdate * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    drone_interfaces__msg__DroneStatusUpdate__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
drone_interfaces__msg__DroneStatusUpdate__Sequence__init(drone_interfaces__msg__DroneStatusUpdate__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  drone_interfaces__msg__DroneStatusUpdate * data = NULL;

  if (size) {
    data = (drone_interfaces__msg__DroneStatusUpdate *)allocator.zero_allocate(size, sizeof(drone_interfaces__msg__DroneStatusUpdate), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = drone_interfaces__msg__DroneStatusUpdate__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        drone_interfaces__msg__DroneStatusUpdate__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
drone_interfaces__msg__DroneStatusUpdate__Sequence__fini(drone_interfaces__msg__DroneStatusUpdate__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      drone_interfaces__msg__DroneStatusUpdate__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

drone_interfaces__msg__DroneStatusUpdate__Sequence *
drone_interfaces__msg__DroneStatusUpdate__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  drone_interfaces__msg__DroneStatusUpdate__Sequence * array = (drone_interfaces__msg__DroneStatusUpdate__Sequence *)allocator.allocate(sizeof(drone_interfaces__msg__DroneStatusUpdate__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = drone_interfaces__msg__DroneStatusUpdate__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
drone_interfaces__msg__DroneStatusUpdate__Sequence__destroy(drone_interfaces__msg__DroneStatusUpdate__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    drone_interfaces__msg__DroneStatusUpdate__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
drone_interfaces__msg__DroneStatusUpdate__Sequence__are_equal(const drone_interfaces__msg__DroneStatusUpdate__Sequence * lhs, const drone_interfaces__msg__DroneStatusUpdate__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!drone_interfaces__msg__DroneStatusUpdate__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
drone_interfaces__msg__DroneStatusUpdate__Sequence__copy(
  const drone_interfaces__msg__DroneStatusUpdate__Sequence * input,
  drone_interfaces__msg__DroneStatusUpdate__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(drone_interfaces__msg__DroneStatusUpdate);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    drone_interfaces__msg__DroneStatusUpdate * data =
      (drone_interfaces__msg__DroneStatusUpdate *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!drone_interfaces__msg__DroneStatusUpdate__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          drone_interfaces__msg__DroneStatusUpdate__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!drone_interfaces__msg__DroneStatusUpdate__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
