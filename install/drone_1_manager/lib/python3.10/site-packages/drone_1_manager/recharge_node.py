...
class DroneNode(Node):
    ...
    def write_simulated_battery_percentage(self, percentage):
        try:
            with open("/tmp/sim_voltage_1.txt", "w") as f:
    ...
    def update_logic(self):
        ...
        if self.status == 'idle':
            battery_low = should_return_to_base(self.battery, self.distance_to(self.base_position), self.fixed_drain_rate, self.speed)
# 🟡 Only consider water level if all waypoints are visited
            water_empty = (
